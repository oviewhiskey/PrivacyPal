...
import { Shield, LogOut, Users, FileText, BarChart3, Plus, Trash2, Share2, History } from 'lucide-react';
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <div className="text-sm text-gray-500 bg-blue-50 px-3 py-2 rounded-lg border border-blue-200">
                  <span className="font-medium">Note:</span> Full audit logs are securely stored in the backend for enhanced privacy and security compliance.
                </div>
              </div>
                ...
                {auditLogs.length === 0 ? (
                  <Card>
                    <CardContent className="text-center py-8">
                      ...
                      <p className="text-gray-600">No audit logs recorded yet.</p>
                    </CardContent>
                  </Card>
                ) : (
                          ...
                          <div className="flex-1">
                            <div className="flex items-center space-x-2 mb-1">
                              <span className={`text-xs px-2 py-1 rounded ${
                                ...
                                log.action === 'login_failure' ? 'bg-red-100 text-red-700' :
                                log.action === 'logout' ? 'bg-blue-100 text-blue-700' :
                                'bg-gray-100 text-gray-700'
                              }`}>
                                {log.action.replace('_', ' ').toUpperCase()}
                              </span>
                              <span className="text-sm font-medium text-gray-900">{maskEmail(log.userEmail)}</span>
                              {log.userLevel && (
                                <span className="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded">
                                  {log.userLevel}
                                </span>
                            ...
                            <p className="text-xs text-gray-400">
                              User Agent: {log.userAgent?.substring(0, 100)}...
                            </p>
                          </div>
                        </div>
                      </CardContent>
